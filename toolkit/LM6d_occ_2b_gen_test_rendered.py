...
rendered_pose_path = "%s/LM6d_occ_%s_{}_test_rendered_pose_{}.txt" % (
    os.path.join(LM6d_root, 'syn_poses'), version)

# output_path
rendered_root_dir = os.path.join(
    cur_dir, '../data/LINEMOD_6D/LM6d_occ_render_v1/data/rendered/occ_test')
...
def main():
    ...
    for class_idx, class_name in idx2class.items():
        ...
        for set_type in ['val']:
            with open(
                    os.path.join(real_set_root, '{}_{}.txt'.format(
                        class_name, 'val')), 'r') as f:
                    ...
                    if real_index in test_real_list:
                        if inner_idx == 0:
                            val_pair.append("{} {}/{}/{}_{}".format(
                                real_index, 'occ_test', class_name,
                                real_prefix, inner_idx))
...
def check_real_rendered():
    ...
    real_dir = os.path.join(cur_dir,
                            '../data/LINEMOD_6D/LM6d_occ_render_v1/data/real')

    for class_idx, class_name in idx2class.items():
        if class_name != 'duck':
            continue
        print(class_name)
        real_list_path = os.path.join(real_set_root,
                                      "{}_val.txt".format(class_name))
