...
for cls_i, cls_name in enumerate(classes):
    ...
    for set in ['train', 'test']:
        ...
        with open(
                os.path.join(model_set_dir, '{}_{}.txt'.format(cls_name,
                                                               set))) as f:
            model_list = [line.strip() for line in f.readlines()]

        for model_name in model_list:
            image_path = os.path.join(class_real_dir, set,
                                      '{}_0000-color.png'.format(model_name))
            ...
            # plt.show()
            plt.savefig(
                os.path.join(example_render_dir, '{}_{}'.format(
                    set, os.path.basename(image_path))))
