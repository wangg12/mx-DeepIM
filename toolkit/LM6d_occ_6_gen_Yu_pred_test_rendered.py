...
if __name__ == '__main__':
    ...
    keyframe_path = "%s/{}_val.txt" % (os.path.join(
        cur_path, '../data/LINEMOD_6D/LM6d_occ_render_v1/image_set/real'))
    yu_pred_dir = os.path.join(cur_dir,
                               '../data/LINEMOD_6D/results_occlusion_posecnn')
    ...
    real_root_dir = os.path.join(cur_path, '..', 'data', 'LINEMOD_6D',
                                 'LM6d_occ_render_v1', 'data', 'real')
    real_meta_path = "%s/{}-meta.mat" % (real_root_dir)
    rendered_root_dir = os.path.join(cur_path, '..', 'data', 'LINEMOD_6D',
                                     'LM6d_occ_render_v1', 'data', version)
    pair_set_dir = os.path.join(cur_path, '..', 'data', 'LINEMOD_6D',
                                'LM6d_occ_render_v1', 'image_set')
    ...
    for small_class_idx, class_name in enumerate(class_name_list):
        ...
        model_dir = os.path.join(
            cur_dir, '../data/LINEMOD_6D/models/{}'.format(class_name))
        if gen_images:
        ...
        for idx, real_index in enumerate(real_index_list):
            ...
            yu_pred_file = os.path.join(yu_pred_dir,
            ...
            if len(labels) >= 1:
                ...
                if len(proposal_idx) == 1:
                    ...
                    print("{}, {:04d}, {}".format(
                        class_name, yu_idx,
                        se3.calc_rt_dist_m(pose_ori_m, pose_gt)))

                    pose_ori_file = os.path.join(
                        rendered_dir, '{}_{}_{}-pose.txt'.format(
                            class_name, real_prefix_list[idx], 0))
                    ...
                    print("no yu_pred in {}, {:04d}, {}".format(
                        class_name, yu_idx, real_index))
                ...
                all_pair.append("{} {}/{}/{}_{}_{}".format(
                    real_index, video_name_list[idx], class_name, class_name,
                    real_prefix_list[idx], 0))
            else:
                print("no yu_pred in {} {} {}".format(class_name, yu_idx,
                                                      real_index))

            pair_set_file = os.path.join(pair_set_dir,
                                         "yu_val_{}.txt".format(class_name))
