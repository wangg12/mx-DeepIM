...
for cls_i, cls_name in enumerate(classes):
    ...
    with open(
        os.path.join(real_set_dir, "{}_{}_real.txt".format(cls_name, "train")), "w"
    ) as f:
        for line in train_indices:
            f.write(line + "\n")

    with open(
        os.path.join(real_set_dir, "{}_{}_real.txt".format(cls_name, "test")), "w"
    ) as f:
        for line in test_indices:
            f.write(line + "\n")

    with open(
        os.path.join(real_set_dir, "{}_{}_real.txt".format(cls_name, "all")), "w"
    ) as f:
