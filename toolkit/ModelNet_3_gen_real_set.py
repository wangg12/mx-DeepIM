...
for cls_i, cls_name in enumerate(classes):
    ...
    for set in ['train', 'test']:
        real_indices = [
            fn.split('-')[0]
            for fn in os.listdir(os.path.join(class_dir, set)) if 'color' in fn
        ]
        real_indices.sort()
    ...
    test_indices.sort()
    train_indices = [
        '{}/train/{}'.format(cls_name, idx) for idx in train_indices
    ]
    ...
    with open(
            os.path.join(real_set_dir, '{}_{}_real.txt'.format(
                cls_name, 'train')), 'w') as f:
        for line in train_indices:
            f.write(line + '\n')

    with open(
            os.path.join(real_set_dir, '{}_{}_real.txt'.format(
                cls_name, 'test')), 'w') as f:
