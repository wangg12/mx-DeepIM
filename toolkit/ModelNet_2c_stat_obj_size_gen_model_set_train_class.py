...
def stat_obj_size():
    ...
    for cls_i, cls_name in enumerate(sel_classes):
        ...
        if not cls_name in train_cls_models.keys():
            ...
            n_model_list = sorted(n_model_list)
            n_model_list_ = [
    ...
    with open(
        os.path.join(model_set_dir, "all_train_{}.txt".format(num_models)), "w"
    ) as f:
    ...
    with open(
        os.path.join(model_set_dir, "all_test_{}.txt".format(num_models)), "w"
    ) as f:
