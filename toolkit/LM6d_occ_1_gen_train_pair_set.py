...
def main():
    for class_idx, class_name in enumerate(tqdm(classes)):
        ...
        for set_type in ['train']:
            f_origin = os.path.join(origin_set_dir, '{}_{}.txt'.format(
                set_type, class_name))
            pairs = []
            with open(f_origin, 'r') as f:
                for line in f:
                    ...
                    rendered_prefix = rendered_prefix.replace(
                        class_name + '_', '')
                    new_rendered_idx = '{}/{}'.format(rendered_video,
                                                      rendered_prefix)

                    pairs.append('{} {}'.format(new_real_idx,
                                                new_rendered_idx))

            with open(
                    os.path.join(image_set_dir, '{}_{}.txt'.format(
                        set_type, class_name)), 'w') as f:
                for pair in pairs:
                    f.write(pair + '\n')
