...
def gen_real():
    ...
    real_set_dir = os.path.join(modelnet_root,
                                'modelnet_render_v1/image_set/real')
    ...
    for real_i in range(num_real_per_model):
        ...
        count = 0
        while not (up_deg <= up_deg_max
                   and front_deg_min < front_deg < front_deg_max):
            tgt_quat = np.random.normal(0, 1, 4)
            tgt_quat = tgt_quat / np.linalg.norm(tgt_quat)
            if tgt_quat[0] < 0:
                tgt_quat *= -1
            ...
            count += 1
            if count % 100 == 0:
                print(real_i, cls_name, count,
                      "up_deg: {}, front_deg: {}".format(up_deg, front_deg))
...
def test_rotate():
    ...
    im_rot, depth_rot, pose_rot = rotate(angle, rot_axis, pose,
                                         render_machine)  #################

    angle = np.pi / 2
    rot_axis = np.array([1, 0, 0])
    im_rot_1, depth_rot_1, pose_rot_1 = rotate(
        angle, rot_axis, pose_rot, render_machine)  #################
    print(pose_rot_1)
    pz = np.array([0, 0, 1])
