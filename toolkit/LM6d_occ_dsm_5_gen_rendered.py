...
rendered_pose_dir = os.path.join(
    cur_path, '..', 'data/LINEMOD_6D/LM6d_occ_ds_multi/ds_rendered_poses')
# output dir
rendered_root_dir = os.path.join(
    cur_path, '..', 'data/LINEMOD_6D/LM6d_occ_ds_multi/data/rendered')
    ...
    for class_idx, class_name in enumerate(tqdm(classes)):
        ...
        if gen_images:
            ...
            model_dir = os.path.join(
                cur_path, '../data/LINEMOD_6D/models/{}'.format(class_name))
        ...
        for set_type in ['NDtrain']:
            ...
            real_list_path = os.path.join(
                real_set_dir, "NDtrain_real_{}.txt".format(class_name))
...
def check_real_rendered():
    ...
    for class_idx, class_name in enumerate(tqdm(classes)):
        if class_name != 'driller':
            continue
        print(class_name)
        real_list_path = os.path.join(real_set_dir,
                                      "NDtrain_real_{}.txt".format(class_name))
        with open(real_list_path, 'r') as f:
            real_list = [x.strip() for x in f.readlines()]
        for idx, real_index in enumerate(real_list):
            ...
            color_real = read_img(
                os.path.join(real_dir, prefix + '-color.png'), 3)
