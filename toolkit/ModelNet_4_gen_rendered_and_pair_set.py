...
for cls_i, cls_name in enumerate(classes):
    ...
    with open(
        os.path.join(real_set_dir, "{}_{}_real.txt".format(cls_name, "all"))
    ) as f:
    ...
    for real_i, real_idx in enumerate(tqdm(all_indices)):
        ...
        real_pose_path = os.path.join(
            data_dir, cls_name, set, "{}-pose.txt".format(prefix)
        )
        ...
        rendered_pose_path = os.path.join(
            rendered_pose_dir, "{}_0-pose.txt".format(prefix)
        )
        ...
        model_path = os.path.join(
            modelnet40_root, cls_name, set, "{}.obj".format(model_name)
        )
        # print(model_name)
        ...
        if real_number == "0000" and gen_images:
            cmd = (
                "python toolkit/ModelNet_4b_renderer.py --model_path {} --texture_path {} "
        ...
        if set == "train":
            if not model_name in pair_set_dict["train"].keys():
                pair_set_dict["train"][model_name] = [
                    "{} {}".format(real_idx, real_idx + "_0")
                ]
            else:
                pair_set_dict["train"][model_name].append(
                    "{} {}".format(real_idx, real_idx + "_0")
                )
        elif set == "test":
            if not model_name in pair_set_dict["my_val"].keys():
                pair_set_dict["my_val"][model_name] = [
                    "{} {}".format(real_idx, real_idx + "_0")
                ]
            else:
                pair_set_dict["my_val"][model_name].append(
                    "{} {}".format(real_idx, real_idx + "_0")
                )
    ...
    for set, model_idx_dict in pair_set_dict.items():
        for model_name, indices in model_idx_dict.items():
            with open(
                os.path.join(image_set_dir, "{}_{}.txt".format(set, model_name)), "w"
            ) as f:
                for line in indices:
                    f.write(line + "\n")
