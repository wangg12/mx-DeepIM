...
real_dir = os.path.join(cur_path, '..', 'data',
                        'LINEMOD_6D/LM6d_occ_ds_multi/data/real')
real_set_dir = os.path.join(cur_path, '..', 'data',
                            'LINEMOD_6D/LM6d_occ_ds_multi/image_set/real')
...
def cal_occ_rate(real_label, render_real_label, cls_name, binary_mask=False):
    ...
    visible_pixels = np.sum(
        np.logical_and(real_label == real_label_idx, render_real_label == 1))
    ...
    if False:  #occ_rate > 0.85:
        ...
        print('visible: {}, all: {}, occ_rate: {}'.format(
        ...
        plt.title('real')
        plt.show()
    assert occ_rate >= 0, 'visible: {}, all: {}'.format(
        visible_pixels, all_pixels)
    return occ_rate
...
def check_real_render_real():
    ...
    for prefix in indices:
        render_real_color_file = os.path.join(render_real_dir, cls_name,
                                              prefix + "-color.png")
        render_real_depth_file = os.path.join(render_real_dir, cls_name,
                                              prefix + "-depth.png")
        render_real_label_file = os.path.join(render_real_dir, cls_name,
                                              prefix + "-label.png")
        ...
        visible_pixels = np.sum(
            np.logical_and(label_r == classes.index(cls_name) + 1,
                           label_rr == 1))
...
def stat_lm6d_occ_rate():
    for cls_idx, cls_name in enumerate(tqdm(sel_classes)):
        ...
        real_dir = os.path.join(cur_dir,
                                '../data/LINEMOD_6D/LM6d_render_v1/data/real')
        render_real_dir = os.path.join(
            cur_dir, '../data/LINEMOD_6D/LM6d_render_v1/data/render_real')
        set_name = 'val'
        keyframe_path = os.path.join(
            cur_dir, '../data/LINEMOD_6D/image_set',
        ...
        for idx, real_index in enumerate(tqdm(real_index_list)):
            ...
            real_label_file = os.path.join(real_dir, video_name,
                                           prefix + "-label.png")
            render_real_label_file = os.path.join(
                render_real_dir, cls_name, video_name, prefix + "-label.png")

            real_label = read_img(real_label_file, 1)
            render_real_label = read_img(render_real_label_file, 1)

            occ_rate = cal_occ_rate(
                real_label, render_real_label, cls_name, binary_mask=True)
        ...
        print('occ rate stat, mean: {}, std: {}'.format(
            np.mean(occ_rates), np.std(occ_rates)))
        print('high occ: {}, NDtrain: {}, all: {}'.format(
            len(high_occ_list), len(NDtrain_list), len(real_index_list)))
...
def main():
    for cls_idx, cls_name in enumerate(tqdm(sel_classes)):
        print(cls_idx, cls_name)
        keyframe_path = os.path.join(real_set_dir,
                                     "train_real_{}.txt".format(cls_name))
        with open(keyframe_path) as f:
            real_index_list = [x.strip() for x in f.readlines()]
        video_name_list = [x.split('/')[0] for x in real_index_list]  # ./
        real_prefix_list = [x.split('/')[1] for x in real_index_list]
        ...
        with open(
                os.path.join(real_set_dir,
                             'HighOcc_train_real_{}.txt'.format(cls_name)),
                'w') as f:
