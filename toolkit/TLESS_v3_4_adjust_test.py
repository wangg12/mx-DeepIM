...
origin_data_root = os.path.join(
    cur_dir, "../data/TLESS/t-less_v2/test_primesense"
)  # 20 videos
...
def adapt_color_depth_pose_label():
    ...
    for cls_idx, cls_name in enumerate(class_list):
        ...
        model_path = os.path.join(
            TLESS_root, "models/{}/obj_{}_scaled.ply".format(cls_name, cls_name)
        )
        ...
        for video_name in sel_videos:  # 02
            ...
            images = [
                fn
                for fn in os.listdir(os.path.join(origin_data_root, video_name, "rgb"))
                if ".png" in fn
            ]
            ...
            for real_img in tqdm(images):
                old_color_path = os.path.join(
                    origin_data_root, video_name, "rgb/{}".format(real_img)
                )
                assert os.path.exists(old_color_path), old_color_path
                old_depth_path = os.path.join(
                    origin_data_root, video_name, "depth/{}".format(real_img)
                )
