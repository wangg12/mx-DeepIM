...
rendered_pose_path = "%s/TLESS_%s_{}_rendered_pose_{}_{}.txt" % (os.path.join(
    TLESS_root, 'syn_poses_v3'), version)

# output_path
rendered_root_dir = os.path.join(
    TLESS_root, 'TLESS_render_v3/data/rendered_{}/test'.format(version))
...
def main():
    ...
    for class_idx, class_name in enumerate(class_list):
        ...
        model_path = os.path.join(
            TLESS_root, 'models/{}/obj_{}_scaled.ply'.format(
        ...
        for video_name in sel_videos:  # 02
            ...
            for set_type in ['test']:
                with open(
                        os.path.join(
                            real_set_root, '{}_{}_{}.txt'.format(
                                class_name, video_name, 'test')), 'r') as f:
                    test_real_list = [x.strip() for x in f.readlines()]

                with open(
                        rendered_pose_path.format(set_type, class_name,
                                                  video_name)) as f:
                ...
                for idx, real_index in enumerate(tqdm(test_real_list)):
                    ...
                    for inner_idx in range(rendered_per_real):
                        if gen_images:
                            ...
                            ren_depth = (ren_depth * depth_factor).astype(
                                np.uint16)
                        ...
                        val_pair.append("{} test/{}/{}/{}".format(
                            real_index, class_name, video_name, real_prefix))

                test_pair_set_file = os.path.join(
                    pair_set_dir, "my_val_{}_video{}_{}.txt".format(
                        version, video_name, class_name))
                val_pair.sort()
...
def check_real_rendered():
    ...
    for class_idx, class_name in enumerate(class_list):
        if not class_name in sel_classes:
            continue
        print(class_name)
        real_list_path = os.path.join(
            real_set_root, "{}_{}_test.txt".format(class_name, '02'))
        ...
        for idx, real_index in enumerate(real_list):
            ...
            color_real = read_img(
                os.path.join(real_dir, real_index + '-color.png'), 3)
            color_rendered = read_img(
                os.path.join(rendered_root_dir, class_name, '02',
                             prefix + '-color.png'), 3)
