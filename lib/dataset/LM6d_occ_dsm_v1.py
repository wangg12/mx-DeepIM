...
class LM6D_occ_DSM_v1(IMDB):
    def __init__(
        ...
        if len(mask_syn_name) > 0:
            ...
            super(LM6D_occ_DSM_v1, self).__init__(
                "LM6d_occ_dsm_v1", image_set, root_path, devkit_path, result_path
            )  # set self.name
        self.root_path = root_path
        self.devkit_path = os.path.join(
            devkit_path, "../LM6d_occ_dsm"
        )  # ################################ hard coded
        print(
            "*******************LM6d occ dsm v1 devkit path: {}".format(
                self.devkit_path
            )
        )
        ...
        if image_set.startswith("posecnn_val"):
            self.rendered_data_path = os.path.join(
                self.devkit_path, "data", "rendered_val_posecnn"
            )
        elif (
        ...
        if image_set.startswith("train"):
            self.phase = "train"
            if len(mask_syn_name) > 0:
                self.mask_syn_path = os.path.join(
                    self.devkit_path, "data", mask_syn_name
                )
        elif (
        ...
        if image_set.startswith("posecnn_val"):
            print("invalid, no posecnn_val in olm_data_syn")
    ...
    def image_path_from_index(self, index, type, check=True, cls_name=""):
        ...
        if type == "observed":
            ...
            image_file = os.path.join(
                self.gt_observed_data_path, cls_name, index + "-color.png"
            )
        elif type == "rendered":
    ...
    def depth_path_from_index(self, index, type, check=True, cls_name=""):
        ...
        if type == "observed":
            ...
            depth_file = os.path.join(
                self.gt_observed_data_path, cls_name, index + "-depth.png"
            )
        elif type == "rendered":
    ...
    def segmentation_path_from_index(self, index, type, check=True):
        ...
        if type == "gt_observed":
            ...
            seg_class_file = os.path.join(
                self.mask_est_path, self.cur_class, index + "-label.png"
            )  # maybe bug
        if check:
            assert os.path.exists(
                seg_class_file
    ...
    def pose_from_index(self, index, type, cls_name=""):
        ...
        if type == "observed" or type == "gt_observed":
            pose_file = os.path.join(
                self.gt_observed_data_path, cls_name, index + "-pose.txt"
            )
        elif type == "rendered":
