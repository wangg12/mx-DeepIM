...
class LM6D_occ_v1(IMDB):
    def __init__(
        ...
        if len(mask_syn_name) > 0:
            super(LM6D_occ_v1, self).__init__(
                "LM6d_occ_render_v1_" + mask_syn_name,
                image_set,
                root_path,
                devkit_path,
                result_path,
            ...
            super(LM6D_occ_v1, self).__init__(
                "LM6D_occ_render_v1", image_set, root_path, devkit_path, result_path
            )  # set self.name
        ...
        if (
            image_set.startswith("posecnn_val")
            or image_set.startswith("posecnn_vis")
        ...
        if (
            ...
            self.gt_observed_data_path = os.path.join(
                devkit_path, "data", "gt_observed/occ_test"
            )
        ...
        if image_set.startswith("posecnn_val") or image_set.startswith("test_vis"):
            if os.path.exists(os.path.join(devkit_path, "data", "mask_posecnn_v01")):
                ...
                self.mask_est_path = (
                    ""
                )  # os.path.join(devkit_path, 'data', 'mask_posecnn_v01')
        else:
            self.mask_est_path = ""
    ...
    def _load_object_points(self):
        ...
        for cls_idx, cls_name in self.idx2class.items():
            point_file = os.path.join(
                self._linemod_path, "models", cls_name, "points.xyz"
            )
    ...
    def _load_object_diameter_from_extent(self):
        ...
        assert os.path.exists(extent_file), "Path does not exist: {}".format(
            extent_file
        )
    ...
    def image_path_from_index(self, index, type, check=True, cls_name=""):
        ...
        #         self.gt_observed_data_path, cls_name, index.split("/")[1] + "-color.png"
        #     )
        elif type == "rendered":
    ...
    def gt_pairdb(self):
        ...
        :return: imdb[pair_index]['image_observed', 'image_rendered', 'height', 'width',
                                  'pose_observed', 'pose_rendered', 'flipped']
        """
    ...
    def load_render_annotation(self, pair_index):
        ...
        if self.mask_est_path != "":
            pair_rec["mask_observed_est"] = self.segmentation_path_from_index(
                pair_index[0], "observed_est"
            )
            ...
            for iter_i in range(num_iter):
                ...
                for j in range(num):
                    ...
                    if cls_name == "eggbox" and r_dist_est > 90:
                        ...
                        curr_pose_rendered_sym = se3_mul(curr_poses_est[j], RT_z)
                        r_dist_est, t_dist_est = calc_rt_dist_m(
                            curr_pose_rendered_sym, curr_poses_gt[j]
                        )
